...
# from DeepSeek_integration import get_explanation
...
#     - **DeepSeek AI Integration:** Provides clear, AI-generated explanations for flagged transactions upon request.
...
#     - **Detailed Transaction View:** Select a transaction for an in-depth view and generate a DeepSeek explanation on demand.
...
#         # -------------------------
#         # Detailed Transaction View with Button to Generate DeepSeek Explanation
...
#             # Button to generate DeepSeek explanation
#             if st.button("Generate DeepSeek Explanation"):
#                 st.markdown("**DeepSeek Explanation:**")
#                 explanation_placeholder = st.empty()
#                 with st.spinner("DeepSeek is processing the transaction..."):
...
if 'filters_applied' not in st.session_state:
    st.session_state.filters_applied = False
if 'DeepSeek_explanation' not in st.session_state:
    st.session_state.DeepSeek_explanation = None
    ...
    if st.session_state.filters_applied:
        ...
        if not filtered_df.empty:
            ...
            if st.button("Generate DeepSeek Explanation"):
                ...
                with st.spinner("DeepSeek is processing the transaction..."):
                    explanation = get_explanation(selected_row.iloc[0])
                    st.session_state['DeepSeek_explanation'] = explanation
                    displayed_text = ""
                    for char in explanation:
                        displayed_text += char
                        explanation_placeholder.markdown(f"{displayed_text}")
        ...
        # -------------------------
        # Show Generate Report button only after DeepSeek explanation is generated
        if st.session_state.DeepSeek_explanation:
            st.subheader("📄 Generate Report")
            if st.button("Generate Report"):
                class PDF(FPDF):
                    def header(self):
                ...
                pdf.cell(0, 10, "DeepSeek Explanation:", ln=True)
                pdf.set_font("Arial", size=12)
                pdf.multi_cell(0, 10, st.session_state['DeepSeek_explanation'])
                pdf.ln(10)
                pdf.multi_cell(0, 10, "This report summarizes the fraud detection results and includes the DeepSeek generated explanation for the selected transaction.")
